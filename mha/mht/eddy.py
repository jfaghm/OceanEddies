...
class Eddy(object):
	__slots__ = ('Stats', 'Lat', 'Lon', 'Amplitude', 'ThreshFound', 'SurfaceArea', 'Date', 'Cyc',
        'MeanGeoSpeed', 'DetectedBy')
	def __init__(self, stats, lat, lon, amp, thresh, surf_area, date, cyc, geo_speed, detectedby):
...
class Stats(object):
    ...
    def __init__(self, area, extrema, pixelIdxList, intensity, convexImage, boundingBox,
        centroid, pixelList, solidity, extent, orientation, majorAxisLength, minorAxisLength):
        self.Area = area
    ...
    def new_from_mat(stats)
        return Stats(stats.Area[0,0], stats.Extrema, stats.PixelIdxList[:,0],
            stats.Intensity[0,0], stats.ConvexImage, stats.BoundingBox[0,:], stats.Centroid[0,:],
            stats.PixelList, stats.Solidity[0,0], stats.Extent[0,0], stats.Orientation[0,0],
            stats.MajorAxisLength[0,0], stats.MinorAxisLength[0,0])
